...
let arangoDriver = require("./lib/arango.js");
let operations = require("./lib/operations.js");
let buildConditionQuery = require("./lib/build-condition-query");
/**
 * Module state
 */
...
module.exports = {
     ...
     * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     */
    registerDatastore: function (datastoreConfig, models, cb) {
        ...
        //A datastore cannot be registered to two drivers
        ...
        if (!datastoreConfig.host) {
            return done(new Error('Invalid configuration for datastore `' + datastoreName + '`:  Missing "host".)'));
        }
        //Missing port attribute
        if (!datastoreConfig.port) {
            return done(new Error('Invalid configuration for datastore `' + datastoreName + '`:  Missing "port".)'));
        }
        //Missing database attribute
        if (!datastoreConfig.database) {
            return done(new Error('Invalid configuration for datastore `' + datastoreName + '`:  Missing "database".)'));
        }
        registeredDatastores[datastoreName] = arangoDriver.createConnection(datastoreConfig);
        return cb();
    },
     ...
     */
    teardown: function (datastoreName, cb) {
        // Look up the datastore entry (manager/driver/config).
        var dsEntry = registeredDatastores[datastoreName];
        // Sanity check:
        if (_.isUndefined(dsEntry)) {
            return cb(new Error('Consistency violation: Attempting to tear down a datastore (`'+datastoreName+'`) which is not currently registered with this adapter.  This is usually due to a race condition in userland code (e.g. attempting to tear down the same ORM instance more than once), or it could be due to a bug in this adapter.  (If you get stumped, reach out at https://sailsjs.com/support.)'));
        }
     ...
     * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     */
    create: function (datastoreName, query, cb) {
        // Look up the datastore entry (manager/driver/config).
        let connection = registeredDatastores[datastoreName];
        // Sanity check:
        ...
        operations.create(connection, query.using, query.newRecord, (err, records) => {
            if(err) return cb(err);
            //if fetch is not enabled, return nothing
            else if(!query.meta || !query.meta.fetch) return cb();
            //return the record
            else return cb(null, records[0]);
        });
    },
    ...
    createEach: function (datastoreName, query, cb) {
        ...
        //the array of records to be returned to the user
        let arrayOfRecords = [];
        async.eachSeries(query.newRecords, (oneRecord, callback) => {
            operations.create(connection, query.using, oneRecord, (err, records) => {
                if(err) return callback (err);
                else {
                    arrayOfRecords.push(records[0]);
                    return callback();
                }
            });
        }, (err) =>{
            ...
            //return the record
            else return cb(null, arrayOfRecords);
        });
    },
    /**
     *  ╦ ╦╔═╗╔╦╗╔═╗╔╦╗╔═╗
     ...
     * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     */
    update: function (datastoreName, query, cb) {
        // Look up the datastore entry (manager/driver/config).
        let connection = registeredDatastores[datastoreName];
        // Sanity check:
        if (_.isUndefined(connection)) {
        ...
        query.criteria.where = (Object.keys(query.criteria.where).length > 0) ? buildConditionQuery(query.criteria.where) : null;
        operations.update(connection, query.using, query.criteria, query.valuesToSet, cb);
    },
     ...
     * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     */
    destroy: function (datastoreName, query, cb) {
        let connection = registeredDatastores[datastoreName];
        query.criteria.where = (Object.keys(query.criteria.where).length > 0) ? buildConditionQuery(query.criteria.where) : null;
        // Sanity check:
        if (_.isUndefined(connection)) {
        ...
        operations.destroy(connection, query.using, query.criteria, cb);
    },
    ...
    find: function (datastoreName, query, cb) {
        ...
        operations.find(connection, query.using, query.criteria, cb);
    },
    ...
    */
    sendNativeQuery: function(connection, collection, cb) {
